...
| `log-login.js` | Authenticated. Called fire-and-forget by `LoginPanel` after a successful Firebase sign-in. Verifies the just-issued token, writes one `access_logs` row with `action='login'`. Per-uid rate limited (10/min). |
| `log-login-failure.js` | Unauthenticated (auth has just failed). Body `{attempted_email, error_code}`. IP-rate-limited (10/min/IP) to prevent log flooding. Writes `access_logs` with `action='login-failure'`, `actor=null`, and the attempted email + Firebase error code in `extra`. Useful for spotting credential-stuffing or account-enumeration attempts. |
