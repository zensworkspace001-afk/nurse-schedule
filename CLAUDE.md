...
SA penalty function encodes hard rules with high weight (連續上班 >6 天 = 2000, 連續大夜 >3 天 = 1000, forbidden N→D/N→E/E→D = 1000, post-night-not-off = 2000, protected staff on E/N = 500000, FORCE_OFF/FORCE_WORK violation = 1000000, personal health floor breach = 50000) plus stricter-than-law custom rules (RG/RC each ∈ [4,5], monthly work days range, weekly RG+RC rhythm, mixed-work-shift ban = 5000) and soft preferences (isolated rest, OT 6th-day). Early stop + `solver_status='OPTIMAL'` when penalty < `OPTIMAL_THRESHOLD` (default 1000); otherwise `FEASIBLE`. **Because the penalty encodes rules stricter than 勞基法, `final_penalty` can stay > 0 even when the schedule fully passes JS `checkLaborLawCompliance` — the real pass criterion is 0 JS violations, not penalty 0.** The file name remains `main1.py` (Render/Dockerfile reference it) — the previous CP-SAT implementation has been replaced.
